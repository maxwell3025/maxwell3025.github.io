...
function stretchSim() {
  ...
  svg.attr(
    'viewBox',
    `-${(window.innerWidth * 1000) / minDim} -${
      (window.innerHeight * 1000) / minDim
    } ${(window.innerWidth * 2000) / minDim} ${
      (window.innerHeight * 2000) / minDim
    }`
  );
}
...
window.onload = () => {
  ...
  (d3.select('body').node() as HTMLElement).appendChild(gui.domElement);
  //events
  ...
  svg.on('mousemove', event => {
    ...
    if (state == ProgramState.Creating) {
      ...
      coordLabel = coordLabel.text(
        `(${(downX - mouseX).toFixed(2)}, ${(downY - mouseY).toFixed(2)})`
      );
    } else {
      coordLabel = coordLabel.text(
    ...
    //update line
    if (state == ProgramState.Creating) {
      velocityLine = velocityLine.attr('x2', mouseX).attr('y2', mouseY);
    }
  });
